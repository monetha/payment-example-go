...
func main() {
	...
	// Customer showed his intent to purchase Goods or Service
	// Merchant initializes an Order

	log.Print("info: starting example flow: Merchant initializes the order")
	...
	price = math.Exp(big.NewInt(10), big.NewInt(15)) // price = 0.001 ETH

	log.Print("info: Merchant adds order")
	...
	// Customer receives information where to pay and what order to pay for
	customerSession = e.NewSession(customerAccount)
	p = processor.NewProcessor(customerSession, processorContractAddress)
	...
	// Customer validates that the order is created and amount to be paid is according agreement
	if order.State == processor.OrderStateCreated && order.Price.Cmp(price) == 0 {
		...
		log.Printf("info: Order information is incorrect")
		return
	}
